...
import { createSquadron, deleteSquadron, assignPilotToSquadron, adminCreatePilot, editSquadron, togglePilotStatus } from './actions';
import { Shield, Users, Trash2, Plus, Loader2, UserPlus, UserCircle, Edit2, Check, X, UserMinus, UserCheck, AlertTriangle } from 'lucide-react';
...
export function SquadronsClient({ 
  ...
  const [activeTab, setActiveTab] = useState<'SQUADRONS' | 'PILOTS' | 'CREATE_PILOT'>('SQUADRONS');
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {allProfiles
                  ...
                  return (
                    <tr key={profile.id} className={`hover:bg-white/5 transition-colors ${isSuspended ? 'bg-red-900/10 opacity-70' : ''}`}>
                      ...
                      <td className="px-6 py-4">
                        <span className="bg-white/10 text-[var(--color-gaming-accent)] px-2 py-1 rounded text-xs font-bold uppercase tracking-wider">
                          {profile.role}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                ...
                {allProfiles.filter(p => selectedSquadronFilter === 'ALL' || p.squadron_id === selectedSquadronFilter).length === 0 && (
                  <tr>
                    <td colSpan={canManageSquadrons ? 4 : 3} className="px-6 py-8 text-center text-gray-500 font-bold uppercase tracking-widest">
